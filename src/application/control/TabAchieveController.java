...
public class TabAchieveController implements Initializable {
		...
		openfile.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				try {
					...
					Desktop.getDesktop().open(new File(path + "\\logs\\" + project.getProjectName() + project.getId()
		...
		result.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				try {
					...
					path = path + "\\logs\\" + project.getProjectName() + project.getId() + "\\"
							+ project.getCreateTime() + "\\Result\\0_results\\" + name_dir + "-result\\" + name_dir
							+ "-[TIRS].png";
			...
			public void handle(ActionEvent event) {
				...
				if (list_achieve.get(listView_achieve.getSelectionModel().getSelectedIndex()).getSettings()
					...
					try {
						...
						String name_dir = project.getProjectDir().substring(i + 1);
						Desktop.getDesktop()
								.open(new File(path + "\\logs\\" + project.getProjectName() + project.getId() + "\\"
										+ project.getCreateTime() + "\\Result\\1_debugs\\" + name_dir + "-debug"));
					} catch (IOException e) {
						e.printStackTrace();
					}
				} else {
	...
	private void writeInfoToDataBase(ProjectBean project) {
		...
		String path = System.getProperty("user.dir");
		bean.setResultPath(
				path + "\\logs\\" + project.getProjectName() + project.getId() + "\\" + project.getCreateTime());
		DBUtil.insert(bean);
